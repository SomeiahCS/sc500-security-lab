export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/me' && request.method === 'GET') return json({ email, name: identity?.name || null });

    if (url.pathname === '/api/state' && request.method === 'GET') {
      const [progressRows, lessonRows, quizRows, confidenceRows, flashRow] = await Promise.all([
        ...
        env.DB.prepare('SELECT lesson_id, confidence FROM lesson_confidence WHERE user_email = ?').bind(email).all(),
      ...
      const confidence = {};
      for (const row of confidenceRows.results || []) confidence[row.lesson_id] = row.confidence;

      return json({ progress, lessons, scores, confidence, flashcards: Number(flashRow?.reviewed_count || 0) });
    }
    ...
    if (url.pathname === '/api/progress' && request.method === 'POST') {
      ...
      await env.DB.prepare(`INSERT INTO user_progress (user_email, track_id, completed, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)
    ...
    if (url.pathname === '/api/lesson' && request.method === 'POST') {
      ...
      await env.DB.prepare(`INSERT INTO lesson_progress (user_email, lesson_id, completed, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)
    ...
    if (url.pathname === '/api/confidence' && request.method === 'POST') {
      ...
      if (!lessonId || !Number.isInteger(confidence) || confidence < 1 || confidence > 4) return json({ error: 'Invalid confidence payload' }, 400);
      await env.DB.prepare(`INSERT INTO lesson_confidence (user_email, lesson_id, confidence, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)
        ON CONFLICT(user_email, lesson_id) DO UPDATE SET confidence = excluded.confidence, updated_at = CURRENT_TIMESTAMP`)
        .bind(email, lessonId, confidence).run();
      return json({ ok: true });
    }
    ...
    if (url.pathname === '/api/quiz' && request.method === 'POST') {
      ...
      await env.DB.prepare(`INSERT INTO quiz_scores (user_email, track_id, score, attempts, updated_at) VALUES (?, ?, ?, 1, CURRENT_TIMESTAMP)
    ...
    if (url.pathname === '/api/flashcards' && request.method === 'POST') {
      ...
      await env.DB.prepare(`INSERT INTO user_stats (user_email, reviewed_count, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)
...
function cleanId(value) { if (typeof value !== 'string') return null; const trimmed = value.trim(); return /^[a-z0-9_-]{1,64}$/i.test(trimmed) ? trimmed : null; }
function cleanLessonId(value) { if (typeof value !== 'string') return null; const trimmed = value.trim(); return /^[a-z0-9_-]+:[0-9]{1,3}$/i.test(trimmed) ? trimmed : null; }
