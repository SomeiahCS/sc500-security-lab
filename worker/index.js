export default {
  async fetch(request, env, ctx) {
    ...
    if (!ctx.access) {
      return json({ error: 'Access authentication required' }, 401);
    }

    const identity = await ctx.access.getIdentity();
    const email = identity?.email;
    if (!email) {
      return json({ error: 'Authenticated identity has no email' }, 403);
    }
    ...
    if (url.pathname === '/api/state' && request.method === 'GET') {
      const [progressRows, quizRows, flashRow] = await Promise.all([
      ...
      return json({
        progress,
        scores,
        flashcards: Number(flashRow?.reviewed_count || 0)
      });
    }
    ...
    if (url.pathname === '/api/progress' && request.method === 'POST') {
      ...
      await env.DB.prepare(`
        ...
        ON CONFLICT(user_email, track_id) DO UPDATE SET
          completed = excluded.completed,
          updated_at = CURRENT_TIMESTAMP
      `).bind(email, trackId, completed ? 1 : 0).run();
    ...
    if (url.pathname === '/api/quiz' && request.method === 'POST') {
      ...
      if (!trackId || !Number.isFinite(score) || score < 0 || score > 100) {
    ...
    if (url.pathname === '/api/flashcards' && request.method === 'POST') {
      ...
      await env.DB.prepare(`
        ...
        ON CONFLICT(user_email) DO UPDATE SET
          reviewed_count = user_stats.reviewed_count + excluded.reviewed_count,
          updated_at = CURRENT_TIMESTAMP
