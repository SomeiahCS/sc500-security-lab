...
localStorage.setItem = function(key, value) {
  ...
  if (key === 'sc500-progress-v3') {
    ...
    for (const trackId of ids) {
      if (!!before[trackId] !== !!after[trackId]) postJson('/api/progress', { track_id: trackId, completed: !!after[trackId] });
    }
  }
  ...
  if (key === 'sc500-lessons-v3') {
    ...
    for (const lessonId of ids) {
      if (!!before[lessonId] !== !!after[lessonId]) postJson('/api/lesson', { lesson_id: lessonId, completed: !!after[lessonId] });
    }
  }
  ...
  if (key === 'sc500-scores-v3') {
    ...
    for (const [trackId, score] of Object.entries(after)) {
      if (before[trackId] !== score) postJson('/api/quiz', { track_id: trackId, score: Number(score) });
    }
  }
...
async function loadCloudState() {
  ...
  try {
    ...
    const changed = JSON.stringify(localProgress)!==JSON.stringify(mergedProgress) || JSON.stringify(localLessons)!==JSON.stringify(mergedLessons) || JSON.stringify(localScores)!==JSON.stringify(mergedScores) || Number(localStorage.getItem('sc500-flashcards-v3')||0)!==mergedFlash;
